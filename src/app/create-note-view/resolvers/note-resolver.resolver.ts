...
export class NoteResolver implements Resolve<NoteDto> {
  ...
  constructor(private noteService: NotesRestControllerService,
    private router: Router) {

  }

  resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<NoteDto> {
    ...
    return this.noteService.getNoteById(id).pipe(catchError(error => {
      this.router.navigate(['error404']);
      return EMPTY
    })
    );
